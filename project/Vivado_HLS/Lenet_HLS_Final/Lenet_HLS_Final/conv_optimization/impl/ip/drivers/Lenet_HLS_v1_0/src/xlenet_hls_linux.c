...
typedef struct {
    ...
    XLenet_hls_uio_map maps[ MAX_UIO_MAPS ];
} XLenet_hls_uio_info;

/***************** Variable Definitions **************************************/
static XLenet_hls_uio_info uio_info;
...
static int uio_info_read_name(XLenet_hls_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XLenet_hls_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XLenet_hls_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XLenet_hls_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XLenet_hls_Initialize(XLenet_hls *InstancePtr, const char* InstanceName) {
	XLenet_hls_uio_info *InfoPtr = &uio_info;
